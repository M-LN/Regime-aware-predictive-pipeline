...
class DriftDetector:
    ...
    def set_reference_distributions(self) -> None:
        ...
        logger.info(
            "Reference distributions set with %d features", len(self.reference_features)
        )

    def check_drift(self, auto_update_reference: bool = False) -> Dict[str, any]:
        """
        ...
        if (
            ...
            and len(self.prediction_buffer) >= self.detection_window
        ):
            current_preds = np.array(
                list(self.prediction_buffer)[-self.detection_window :]
            )
            pred_drift = self._detect_feature_drift(
                current_preds, self.reference_predictions, "predictions"
            )
        ...
        if (
            self.reference_regime_counts is not None
            and len(self.regime_buffer) >= self.detection_window
        ):
            current_regimes = np.array(
                list(self.regime_buffer)[-self.detection_window :]
            )
                ...
                results["alerts"].append(
                    {
                        ...
                        "reference_distribution": regime_drift[
    ...
    def _detect_feature_drift(
        self, current: np.ndarray, reference: np.ndarray, name: str
    ) -> Dict[str, any]:
        """
    ...
    def _compute_kl_divergence(
        self, reference: np.ndarray, current: np.ndarray, n_bins: int = 50
    ) -> float:
        """
    ...
    def _detect_regime_drift(self, current_regimes: np.ndarray) -> Dict[str, any]:
        ...
        # Chi-square test
        all_regimes = set(
            list(current_counts.keys()) + list(self.reference_regime_counts.keys())
        )
        observed = [current_counts.get(r, 0) for r in sorted(all_regimes)]
        expected = [
            self.reference_regime_counts.get(r, 0) * (total_current / total_ref)
        ...
        drift_detected = (
            chi2_stat > 10.0
        )  # Threshold for chi-square with 2 df at p<0.01

        return {
            "drift_detected": drift_detected,
            "severity": (
                "high" if chi2_stat > 20 else "medium" if drift_detected else "low"
            ),
