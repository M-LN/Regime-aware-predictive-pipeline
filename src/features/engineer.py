    ...
    def _add_rolling_statistics(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_cols:
            for window in self.rolling_windows:
                # Mean and std
                features[f'{col}_ma_{window}h'] = df[col].rolling(window=window, min_periods=1).mean()
                features[f'{col}_std_{window}h'] = df[col].rolling(window=window, min_periods=1).std(ddof=0)
                
                # Min and max
                features[f'{col}_min_{window}h'] = df[col].rolling(window=window, min_periods=1).min()
                features[f'{col}_max_{window}h'] = df[col].rolling(window=window, min_periods=1).max()
    ...
    def _add_volatility_features(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        for col in numeric_cols:
            for window in self.volatility_windows:
                ...
                features[f'{col}_cv_{window}h'] = rolling_std / (rolling_mean + 1e-6)  # Add epsilon for safety
...
class FeatureScaler:
    ...
    def fit(self, df: pd.DataFrame, feature_cols: List[str]) -> None:
        ...
        self.logger.info(f"Fitted {self.method} scaler for {len(feature_cols)} features")
...
class FeatureStore:
    ...
    def save_features(self, 
                      df: pd.DataFrame, 
                      version: str = "1.0.0",
                      feature_set_name: str = "energy_features") -> bool:
        """
